...
import { AssemblyFile, BootJsonData, ContentFile, PdbFile } from "./BootConfig";
import {
  WebAssemblyStartOptions,
  WebAssemblyBootResourceType,
} from "./WebAssemblyStartOptions";
import { responseProgress } from "../StreamingExtensions";
...
export class WebAssemblyResourceLoader {
  ...
  private requestedPayload: number = 0;
  ...
  static async initAsync(
    ...
    if(!startOptions.packages || startOptions.packages.length === 0) {
      throw new Error("No wasm packages provided. Aborting execution");
    }
  ...
  *loadPdbResource(
  ...
  ): IterableIterator<LoadingResource> {
  ...
  loadResources(
    ...
    return resources.map(({ name, hash, size }) =>
      this.loadResource(name, url(name), hash, size, resourceType)
    );
  }
  ...
  logToConsole() {
    ...
    console.groupCollapsed(
      `%cwasm-sdk%c Loaded ${toDataSizeString(
        totalResponseBytes
  ...
  private async loadResourceWithoutCaching(
    name: string,
    url: string,
    contentHash: string,
    resourceType: WebAssemblyBootResourceType
    ...
    if (this.startOptions.loadingProgress) {
      let loadingProgress = this.startOptions.loadingProgress;

      return await responseProgress(response, (x) => {
        this.currentPayload += x;
        loadingProgress(
          this.currentPayload,
...
async function getCacheToUseIfEnabled(
  ...
  // WasmSdk application running on the same origin. We need this so that we're free
  ...
  const relativeBaseHref = self.location.pathname;
  const cacheName = `wasmsdk-resources-${relativeBaseHref}`;
