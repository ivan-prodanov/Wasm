...
class Driver {
	...
	int Run (string[] args) {
		...
		if (!emit_ninja) {
			...
			var bcl_dir = Path.GetFullPath(Path.Combine(out_prefix, deploy_prefix));
			var currentDirectory = Path.GetFullPath(Directory.GetCurrentDirectory());
			if (currentDirectory != bcl_dir)
            {
            ...
            {
				DirectoryInfo dir = new DirectoryInfo(bcl_dir);
		...
		var linkerSearchPaths = string.Join(" ", root_search_paths.Concat(bcl_prefixes).Distinct().Select(p_ => $"-d \"{p_}\" "));
